...
APPLE_HELPER="$OUTPUT_DIR/jst-apple-intelligence"
...
[ -x "$APPLE_HELPER" ] || {
    echo "error: $APPLE_HELPER not found — run scripts/build-macos-release.sh first" >&2
    exit 1
}
...
codesign --force --options runtime --timestamp --sign "$SIGNING_IDENTITY" "$APPLE_HELPER"
codesign --verify --strict --verbose=2 "$APPLE_HELPER"
...
VERIFIED_APPLE_HELPER="$VERIFY_DIR/jst-macos-universal/jst-apple-intelligence"
codesign --verify --strict --verbose=2 "$VERIFIED_BINARY"
codesign --verify --strict --verbose=2 "$VERIFIED_APPLE_HELPER"
